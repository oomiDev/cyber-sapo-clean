...
document.addEventListener('DOMContentLoaded', () => {
    ...
    const safeAddEventListener = (id, event, handler) => {
        ...
        if (element) {
            ...
            // Advertir si un elemento no se encuentra, en lugar de romper el script
    ...
    // Gestión de Máquinas (pestaña original)
    safeAddEventListener('formMaquina', 'submit', guardarMaquina);
    safeAddEventListener('clearFormMaquina', 'click', () => document.getElementById('formMaquina').reset());
    safeAddEventListener('localMaquina', 'change', seleccionarLocal);

    // --- CARGA INICIAL DE DATOS (ORDEN CORRECTO) ---

    // 1. Cargar datos para los SELECTS de los formularios y filtros
    cargarRegionesParaSelects();
    cargarTiposEstablecimientoParaSelects();
    cargarLocalesParaSelect();

    // 2. Cargar datos para las TABLAS principales
    cargarRegiones();
    cargarLocales();
    cargarMaquinas();
    cargarTiposMaquina();
...
async function cargarRegiones() {
    try {
        const activaInput = document.getElementById('filtroEstadoRegiones');
        const ordenInput = document.getElementById('ordenRegiones');

        const activa = activaInput ? activaInput.value : '';
...
function actualizarTablaRegiones(regiones) {
    ...
    tbody.innerHTML = regiones.map(region => {
        ...
        <tr>
            ...
            <td>
                <button class="btn btn-sm btn-outline-primary btn-action" onclick="editarRegion('${region.codigoRegion}')" title="Editar">
                    <i class="fas fa-edit"></i>
                </button>
                <button class="btn btn-sm btn-outline-${region.activa ? 'warning' : 'success'} btn-action" onclick="toggleRegion('${region.codigoRegion}', ${!region.activa})" title="${region.activa ? 'Desactivar' : 'Activar'}">
                    <i class="fas fa-${region.activa ? 'pause' : 'play'}"></i>
                </button>
                <button class="btn btn-sm btn-outline-danger btn-action" onclick="eliminarRegion('${region.codigoRegion}')" title="Eliminar">
...
async function guardarRegion(event) {
    ...
    try {
        ...
        if (response.ok) {
            ...
            mostrarError(`Error: ${resultado.mensaje || resultado.error}`);
        }
    } catch (error) {
        console.error('Error:', error);
        mostrarError(`Error de conexión al guardar la región`);
    }
}
    ...
    try {
        ...
        if (response.ok) {
            ...
            const selects = [
                ...
                'regionMaquina', 
                'filtro-region-local',
                'filtroRegionMaquinas' // Asegúrate que este ID existe en tu HTML
            ];
            ...
            selects.forEach(selectId => {
                ...
                if (select) {
                    ...
                    opciones.forEach(opcion => {
                        select.innerHTML += `<option value="${opcion.value}" style="color: ${opcion.color}">${opcion.text}</option>`;
                    });
...
async function toggleRegion(codigo, activar) {
    try {
        ...
        if (response.ok) {
            mostrarExito(`Región ${activar ? 'activada' : 'desactivada'}`);
            cargarRegiones();
            cargarRegionesParaSelects();
        } else {
...
function editarRegion(codigo) {
    ...
    document.getElementById('region-id').value = region.codigoRegion; // Usar el código como ID para la edición
...
async function eliminarRegion(codigo) {
    if (!confirm('¿Está seguro de eliminar esta región? Esta acción no se puede deshacer.')) return;

    try {
        const response = await fetch(`/api/regiones/${codigo}`, { method: 'DELETE' });
        const resultado = await response.json();
...
function seleccionarLocal() {
    ...
    if (local && local.ubicacion) {
        // Asignar valores a los campos de la máquina
        const regionSelect = document.getElementById('regionMaquina');
        const ciudadInput = document.getElementById('ciudadMaquina');
        const direccionInput = document.getElementById('direccionMaquina');

        if (regionSelect) regionSelect.value = local.ubicacion.region || '';
        if (ciudadInput) ciudadInput.value = local.ubicacion.ciudad || '';
        if (direccionInput) direccionInput.value = local.ubicacion.direccion || '';
    }
}
...
async function cargarMaquinas() {
    try {
        const regionInput = document.getElementById('filtroRegionMaquinas');
        const estadoInput = document.getElementById('filtroEstadoMaquinas');
...
function actualizarTablaMaquinas(maquinas) {
    ...
    tbody.innerHTML = maquinas.map(maquina => `
        <tr>
            ...
            <td>
                <button class="btn btn-sm btn-outline-primary" onclick="editarMaquina('${maquina._id}')"><i class="fas fa-edit"></i></button>
                <button class="btn btn-sm btn-outline-danger" onclick="eliminarMaquina('${maquina._id}')"><i class="fas fa-trash"></i></button>
            </td>
        </tr>
    `).join('');
}
...
async function guardarMaquina(event) {
    ...
    const data = {
        ...
        local: document.getElementById('localMaquina').value,
    };

    const url = esEdicion ? `/api/maquinas/${id}` : '/api/maquinas';
    const method = esEdicion ? 'PUT' : 'POST';
    ...
    try {
        ...
        const resultado = await response.json();
        if (response.ok) {
            mostrarExito(`Máquina ${esEdicion ? 'actualizada' : 'creada'} con éxito.`);
            document.getElementById('formMaquina').reset();
            document.getElementById('idMaquina').value = '';
            cargarMaquinas();
        } else {
            mostrarError(resultado.error || 'Error al guardar la máquina.');
        }
    } catch (error) {
        console.error('Error:', error);
        mostrarError('Error de conexión al guardar.');
...
function editarMaquina(id) {
    const maquina = maquinasData.find(m => m._id === id);
    if (!maquina) return;

    document.getElementById('idMaquina').value = maquina._id;
    document.getElementById('codigoMaquina').value = maquina.codigo;
    document.getElementById('nombreMaquina').value = maquina.nombre;
    document.getElementById('localMaquina').value = maquina.local?._id || maquina.local;
    document.getElementById('formMaquina').scrollIntoView({ behavior: 'smooth' });
}
...
async function eliminarMaquina(id) {
    ...
    try {
        ...
        if (response.ok) {
            mostrarExito('Máquina eliminada con éxito.');
            cargarMaquinas();
        } else {
            const resultado = await response.json();
            mostrarError(resultado.error || 'Error al eliminar.');
        }
    } catch (error) {
        console.error('Error:', error);
        mostrarError('Error de conexión al eliminar.');
...
function mostrarError(mensaje) {
    ...
    alerta.style.zIndex = '1050'; // Asegurar que esté por encima de otros elementos
    alerta.role = 'alert';
    alerta.innerHTML = `
...
// Cancelar edición de local y limpiar formulario
...
function editarLocal(codigoLocal) {
    ...
    // Rellenar formulario
    document.getElementById('local-id').value = local.codigoLocal; // Usar código como ID para la edición
    ...
    // Mostrar botón de cancelar y cambiar título
...
async function eliminarLocal(codigoLocal) {
    if (!confirm(`¿Está seguro de que desea eliminar el local con código ${codigoLocal}? Esta acción no se puede deshacer.`)) {
        return;
    }

    try {
        const response = await fetch(`/api/locales/${codigoLocal}`, { method: 'DELETE' });
        const resultado = await response.json();
        ...
        if (response.ok) {
            ...
            mostrarError(`Error al eliminar el local: ${resultado.error}`);
        }
    } catch (error) {
        console.error('Error de conexión:', error);
        mostrarError('Error de conexión al intentar eliminar el local.');
    }
}
        const element = document.getElementById(id);
        if (element) {
